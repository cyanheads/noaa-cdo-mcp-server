    ...
    try {
      ...
      if (err instanceof McpError && err.code === JsonRpcErrorCode.InvalidParams) {
        throw ctx.fail('validation_error', err.message, {
          recovery: {
            hint: 'Verify filter IDs — use noaa_climate_list_data_categories to list valid datacategoryId values and noaa_climate_list_datasets to list valid datasetId values.',
          },
        });
      }
      throw err;
